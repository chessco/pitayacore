...
export class EcommerceService {
  ...
  async adjustStock(
    ...
    const product = await this.db.mysql.product.findUnique({
      where: { id: productId },
    });
  ...
  async createOrder(tenantId: string, data: any) {
    ...
    return this.db.mysql.$transaction(async (tx) => {
      ...
      if (contact) {
        const contactFresh = await tx.contact.findUnique({
          where: { id: contact.id },
        });
        if (contactFresh) {
    ...
    return new Promise((resolve, reject) => {
      ...
      const run = async () => {
        try {
          ...
          if (branding?.logoUrl) {
            try {
              const response = await axios.get(branding.logoUrl, {
          ...
          doc
            ...
            .fontSize(22)
            .font('Helvetica-Bold')
            .text(order.tenant.name, 110, 50);
          doc
            .fontSize(10)
            .font('Helvetica')
            .text('Factura / Remisión de Venta', 110, 80);
          doc
            .fontSize(9)
            .fillColor('#64748b')
            .text(`ID: #${order.id.slice(0, 8)}`, 110, 95);

          doc
            .fillColor('#000000')
            .fontSize(10)
            .text(`Fecha: ${order.createdAt.toLocaleDateString()}`, 400, 50, {
              align: 'right',
            });
          doc.moveDown(4);
          ...
          doc
            ...
            .fillColor('#000000')
            .font('Helvetica-Bold')
            .text(order.customerName, 50, 195);
          doc
            .fontSize(10)
            .font('Helvetica')
            .text(order.email || 'N/A', 50, 210);
          ...
          doc
            .fillColor('#64748b')
            .fontSize(10)
            .text('TOTAL A PAGAR', 365, summaryY + 20);
          doc
            .fillColor('#0f172a')
            .fontSize(24)
            .font('Helvetica-Bold')
            .text(`$${order.total.toFixed(2)}`, 365, summaryY + 40);
          ...
          doc
            ...
            .fillColor('#94a3b8')
            .text(
              'Gracias por su preferencia. Esta es una remisión electrónica generada por PitayaCore AI.',
              50,
              footerY + 15,
              { align: 'center' },
            );
          doc.text(
            `${order.tenant.name} - ${order.tenant.slug}.pitayacore.io`,
            50,
            footerY + 30,
            { align: 'center' },
          );
  ...
  async getProfitabilityReport(tenantId: string) {
    ...
    orders.forEach((order) => {
      ...
      if (order.capsuleId && order.capsule) {
        ...
        if (!attributionStats[capId]) {
          attributionStats[capId] = {
            name: order.capsule.title,
            revenue: 0,
            orders: 0,
          };
        }
    ...
    return {
      ...
      topProducts,
      attribution: Object.entries(attributionStats)
        .map(([id, stats]) => ({
          id,
          ...stats,
        }))
  ...
  async getExchangeRate() {
    ...
    return 17.5; // Mock rate USD to MXN
  }
  ...
  async createPaymentIntent(slug: string, amount: number) {
    const tenant = await this.db.mysql.tenant.findUnique({ where: { slug } });
    if (!tenant || !tenant.stripeApiKey)
      throw new Error('Payments not configured for this store');
  ...
  async getStockPredictions(tenantId: string) {
    ...
    orders.forEach((order) => {
      order.items.forEach((item) => {
        salesVolume[item.productId] =
          (salesVolume[item.productId] || 0) + item.quantity;
      });
    });
    ...
    return products.map((p) => {
      ...
      const daysLeft =
        dailyRate > 0 ? Math.floor(p.stock / dailyRate) : Infinity;
  ...
  async getAiInsights(tenantId: string) {
    ...
    ${predictions
      ...
      .map(
        (p: any) =>
          `- ${p.name}: Se agota en ${p.daysLeft} días (Stock: ${p.currentStock})`,
      )
      .join('\n')}
