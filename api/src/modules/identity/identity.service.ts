  ...
  async getStatus() {
    ...
    return {
      tables: {
        ...
        permissions: { count: permissions, status: permissions > 0 ? 'ok' : 'empty' },
        rolePermissions: { count: rolePermissions, status: rolePermissions > 0 ? 'ok' : 'empty' },
        userRoles: { count: userRoles, status: userRoles > 0 ? 'ok' : 'empty' },
        verticalRoles: { count: verticalRoles, status: verticalRoles > 0 ? 'ok' : 'empty' },
        userContexts: { count: userContexts, status: userContexts > 0 ? 'ok' : 'empty' },
        features: { count: features, status: features > 0 ? 'ok' : 'empty' },
        organizations: { count: organizations, status: organizations > 0 ? 'ok' : 'empty' },
  ...
  async seed() {
    ...
    try {
      ...
      const rolePermAssignments: Record<string, string[]> = {
        ...
        brand_manager: ['vision:brands:create', 'vision:brands:read', 'vision:characters:read'],
        campaign_manager: ['vision:campaigns:create', 'vision:campaigns:read', 'vision:characters:read'],
        school_admin: [
          'lumo:schools:manage', 'lumo:classrooms:manage',
          'lumo:students:manage', 'lumo:lessons:manage', 'lumo:evaluations:manage',
          'users:create', 'users:read', 'users:update',
        ],
        teacher: ['lumo:classrooms:manage', 'lumo:lessons:manage', 'lumo:evaluations:manage', 'lumo:students:manage'],
        parent: ['lumo:evaluations:manage'],
        student: [],
        operator: ['mando:campaigns:manage', 'mando:analytics:read'],
        analyst: ['mando:analytics:read'],
        farm_manager: ['aqua:farms:manage', 'aqua:ponds:manage', 'aqua:cycles:manage'],
      ...
      for (const [roleSlug, permKeys] of Object.entries(rolePermAssignments)) {
        ...
        for (const permKey of permKeys) {
          const perm = await this.db.mysql.permission.findUnique({ where: { key: permKey } });
          if (!perm) continue;
      ...
      for (const [verticalSlug, roleSlugs] of Object.entries(verticalRoleAssignments)) {
        const vertical = await this.db.mysql.vertical.findUnique({ where: { slug: verticalSlug } });
        if (!vertical) { log(`Vertical '${verticalSlug}' not found, skipping`); continue; }
