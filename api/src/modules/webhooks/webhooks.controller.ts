...
export class WebhooksController {
  ...
  async handleFlowIncoming(
    ...
    if (!validKey || internalKey !== validKey) {
      this.logger.error(
        `Unauthorized access attempt with key: ${internalKey?.substring(0, 5)}...`,
      );
    ...
    // but we can log it
    console.log(
      `[Pitayacore Webhook] Received message for tenant ${tenantId} from ${payload.userId}. Content: ${payload.content}`,
    );
